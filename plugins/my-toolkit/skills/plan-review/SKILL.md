...
description: >
  This skill should be used when the user runs "/my-toolkit:plan-review" to review an implementation plan.
...
用户执行 `/my-toolkit:plan-review` 命令时激活。参数格式：`[plan 文件路径] [并发数]`。
...
启用 `my-toolkit:requesting-plan-review` 技能，按指定并发数派发 `my-toolkit:plan-reviewer` subagent。
...
**所有 agent 完成后**，启用 `my-toolkit:receiving-plan-review` 技能处理反馈：
