...
description: >
  This skill should be used when the user runs "/my-toolkit:spec-review" to review a spec document.
...
对需求规格文档进行并发多 agent 审查，自动合并去重反馈并修复问题。

## 触发方式

用户执行 `/my-toolkit:spec-review` 命令时激活。参数格式：`[spec 文件路径] [并发数]`。
...
启用 `my-toolkit:requesting-spec-review` 技能，按指定并发数派发 `my-toolkit:spec-reviewer` subagent。

替换模板占位符：
- `{SPEC_FILE_PATH}` — Spec 文档路径
- `{ORIGINAL_REQUEST}` — 用户的原始功能描述
- `{PROJECT_CONTEXT}` — 项目架构关键信息（框架、约定、约束）

### 2. 合并反馈（并发数 > 1 时）

**所有 agent 完成后**，启用 `my-toolkit:receiving-spec-review` 技能处理反馈：

1. **合并去重** — 收集所有问题，识别共识（2+ agent 标记同一问题）
2. **验证** — 将每个问题与原始用户需求交叉比对
3. **评估** — 区分真实缺陷、误解、YAGNI 建议和误报
4. **排序** — 共识问题优先，然后 Critical → Important → Minor
5. **修复** — 直接修改 spec 文件，按类别逐项修复

**并发数 = 1 时**，直接根据单个 agent 的反馈修复，跳过合并去重。

修复优先级、验证标准和禁止行为由 `receiving-spec-review` 技能定义，遵循其规则执行。
